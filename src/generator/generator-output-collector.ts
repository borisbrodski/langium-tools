 ...
 * }
 * ```
 * Alternatively, create multiple instances of GeneratorOutputCollector() for each output directory (outlet) each.
 * E.g. one for <code>src</code> and one for <code>src-gen</code>:
 *
 * ```ts
 * ...
 * generator(collectorSrc.generatorOutputFor(model1), collectorSrcGen.generatorOutputFor(model1))
 * generator(collectorSrc.generatorOutputFor(model2), collectorSrcGen.generatorOutputFor(model2))
 * ...
 * collectorSrc.writeToDisk("src/")
 * collectorSrcGen.writeToDisk("src-gen/")
 * ```
 *
 * @see {@link GeneratorOutput}
 * @category Generator
 * @since 0.1.0
 */
