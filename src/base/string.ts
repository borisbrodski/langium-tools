 ...
 * useful for defining code blocks or text outputs. It uses Platform newlines (e.g. `\r\n` on Windows).
 *
 ...
 * @returns A formatted string with fixed indentation and Unix-style line breaks (`\n`), without carriage return characters (`\r`).
 */
export function adjustedUnix(staticParts: TemplateStringsArray, ...substitutions: unknown[]): string {
  return adjusted(staticParts, substitutions).replace(/\r/g, '');
}
